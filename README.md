...
`product-video-remake` 是一个离线商品视频复刻编译 Skill。它从商品图片和参考视频中整理商品画像、Video DNA、镜头时间线和生成请求，并在用户提供生成成片后执行结构验收。

该项目不会上传素材、调用视频生成 API、轮询任务或下载生成结果。参考视频只用于本地分析，模型请求只应包含独立成片提示词和目标商品图片。
